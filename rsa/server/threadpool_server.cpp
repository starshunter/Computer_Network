...
void connection_handler(void *socket_desc)
{
    ...
    while( (read_size = recv(sock , client_message , 2000 , 0)) > 0 )
    {
        ...
        puts(client_message);
        char temp[2000], *ptr = 0, temp_len[10];
        ...
        else
        {
            ...
            {
            	char *client_message_ptr = &client_message[0];
            	//puts("ready to decrypt");
            	//puts(client_message_ptr);
            	char *decrypt_message = (char*)rsa_decrypt(client_message_ptr);
            	puts(decrypt_message);
            	char *sender = strtok(decrypt_message, "#");
            	ptr = strtok(NULL, "#");
            	char amount[10];
            	strcpy(amount, ptr);
            	ptr = strtok(NULL, "#");
